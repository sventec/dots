...
def powerline(fg="light", bg="dark"):
    return widget.TextBox(**base(fg, bg), text="|", fontsize=14, padding=2)
...
common_widgets = [
    *workspaces(),
    ...
    # fontsize - 2
    widget.Net(**base(fg="focus"), fontsize=12, interface="wlp61s0", padding=5, format="{down} ↓↑ {up}"),
    # fontsize - 2
    widget.Wlan(**base(fg="focus"), fontsize=12, padding=5, interface="wlp61s0", format="{essid} {percent:2.0%}"),
    widget.CheckUpdates(
    ...
    widget.Battery(
        **base(fg="color2"), battery=0, charge_char="", discharge_char="", full_char="", unknown_char=""
    ),
