...
term = "kitty"
backup_term = "urxvt"
launcher_text = f"{homedir}/.config/rofi/scripts/launcher_t4"
...
keys = [
    Key(key[0], key[1], *key[2:]) for key in [
        ...
        # adi1090x/rofi launcher_text
         ([mod, "shift"], "space", lazy.spawn(launcher_text)),
        ...
        ([mod, "shift"], "Return", lazy.spawn(backup_term)),
        ...
        ([mod, "control"], "x", lazy.spawn([path.join(qtile_path, "screenoff.sh")])),
        # ([mod, "control"], "x", lazy.spawn("sleep 1;xset dpms force off")),

        # Show power menu (rofi-power-menu)
        ([mod], "x", lazy.spawn(f"{homedir}/.config/rofi/scripts/powermenu_t1")),
        ...
        ([], "XF86MonBrightnessDown", lazy.spawn(f"{homedir}/scripts/brightnessControl.sh down")),
        (["shift"], "XF86MonBrightnessUp", lazy.spawn(f"{homedir}/scripts/brightnessControl.sh sup")),
        (["shift"], "XF86MonBrightnessDown", lazy.spawn(f"{homedir}/scripts/brightnessControl.sh sdown")),
        ([], "XF86KbdBrightnessUp",
         lazy.spawn("brightnessctl --device='tpacpi::kbd_backlight' set +10%")
         ),
...
keys.append(
    ...
    KeyChord([mod], "p", [
        ...
        Key([], "g", lazy.spawn(term + " lazygit")),
        Key([], "o", lazy.spawn("obsidian")),
    ], name="Shortcuts")  # type: ignore[reportGeneralTypeIssues]
)
...
keys.append(
    ...
    ], name="Redshift")  # type: ignore[reportGeneralTypeIssues]
)
